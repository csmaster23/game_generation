...
class Attention_Model(nn.Module):
    def __init__(self, embedding_size=60, embedding_size_2=120, verbose=False):
        super().__init__()
...
def do_some_attention(embeddings, trajectories, attention_model, is_child=False, verbose=False):
    att_thresh = 0.3
    if verbose:
...
def update_mask(mask, indices, group_nums, verbose=False):
    if len(indices) == 0:
        return mask
    rows, cols = [], []
...
def create_mask(entity_group_nums, verbose=False):
    if verbose:
...
def get_custom_entity_groups(entity_group_nums, entity_mechanic_nums, verbose=False):
    if verbose:
        print("Entity group nums: %s" % str(entity_group_nums))
