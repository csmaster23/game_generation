...
class Mechanic():
    ...
    def add_to_entity(self, entity, tree_trajectory):
        ...
        if detailed_trajectory["selected_parent_entity"].item() == 0:
            # Child entity
            entity.entity_names.add(self.child_entity_name + str(detailed_trajectory["selected_child_entity"].item()))
            ...
            try:
                self.all_pattern_symbols[action_type][detailed_trajectory["selected_pattern"].item()].append(pattern_symbol)
            except KeyError:
                self.all_pattern_symbols[action_type] = {detailed_trajectory["selected_pattern"].item(): [pattern_symbol]}
        else:
            # Add parent name
            parent_name = self.parent_entity_names[detailed_trajectory["selected_parent_entity"].item()] + str(detailed_trajectory[
                "selected_group"].item())
